...
if (isset($_POST['multi_delete']) && !empty($_POST['ids'])) {
    ...
    $success = "Selected news successfully deleted!";
}
...
if (isset($_GET['delete'])) {
    ...
    if ($db->numRows($check) > 0) {
        if ($db->query("DELETE FROM berita WHERE id = $id")) {
            $success = 'The news has been successfully deleted!';
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['multi_delete'])) {
    ...
    if (!$judul || !$kategori || !$tanggal) {
        $error = "Title, category, and date are required fields!";
    }
        ...
        if ($id > 0) {
            ...
            if ($db->numRows($check) == 0) {
                ...
                if ($db->query($sql)) {
                    ...
                    $error = 'Failed to update the news!';
                }
            }
        } else {
            // INSERT
            ...
            if ($db->query($sql)) {
                $success = 'The news has been successfully added!';
            } else {
                $error = 'Failed to add the news!';
            }
        }
    }
}
...
<div class="bg-white rounded-xl shadow-md p-6 mb-6">
    ...
    <h3 class="text-lg font-bold text-gray-800 mb-4"><?= $editData ? 'Edit' : 'Add' ?> News</h3>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            <label class="block text-gray-700 font-semibold mb-2">Brief Description</label>
        ...
        <!-- QUILL EDITOR -->
        <div class="mb-4">
            <label class="block text-gray-700 font-semibold mb-2">News Content *</label>
        ...
        <div class="grid md:grid-cols-3 gap-4 mb-4">
            ...
            <div>
                <label class="block text-gray-700 font-semibold mb-2">Image
        ...
        <div class="flex justify-end space-x-2">
            <?php if ($editData): ?>
                <a href="berita.php" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-lg">Cancel</a>
            <?php endif; ?>
    ...
    <form method="POST">
        ...
        <div class="p-4 border-t flex justify-between">
            <button type="submit" name="multi_delete" class="px-4 py-2 bg-red-500 text-white rounded-lg"
                onclick="return confirm('Delete all selected items?')">
                Delete Selected
            </button>
