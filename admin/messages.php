...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reply_email'])) {
    ...
    if (!$reply_email) {
        $error = 'The destination email address is invalid.';
    } elseif ($reply_subject === '' || $reply_body === '') {
        $error = 'The subject and content of the reply must be filled in.';
    } else {
        ...
        <div style="font-family: Arial, sans-serif; color:#222;">
            ...
            <p>Greetings,<br>' . htmlspecialchars($from_name) . '</p>
        </div>';
        ...
        if ($sent) {
            $success = 'The reply has been successfully sent.';
    ...
    <div class="divide-y divide-gray-200">
        ...
        <div class="p-6 <?php echo !$msg['is_read'] ? 'bg-blue-50' : ''; ?>">
            ...
            <div class="flex space-x-2">
                ...
                <?php if (!$msg['is_read']): ?>
                <a href="?read=<?php echo $msg['id']; ?>" class="px-4 py-2 bg-green-500 text-white rounded-lg">Mark as Read</a>
                <?php endif; ?>
...
<div id="replyModal" class="fixed inset-0 bg-black bg-opacity-40 hidden items-center justify-center z-50">
    <div class="bg-white w-full max-w-2xl rounded-xl shadow-xl p-6 mx-4">
        <h2 class="text-xl font-bold mb-4">Reply Message</h2>
        ...
        <form method="POST">
            ...
            <div class="mb-4">
                <label class="block">Reply Body</label>
                <textarea name="reply_body" id="reply_body" rows="6" class="w-full border px-3 py-2" required></textarea>
            </div>

            <div class="flex justify-end space-x-2">
                <button type="button" onclick="closeReplyModal()" class="px-4 py-2 bg-gray-300">Cancel</button>
                <button type="submit" class="px-4 py-2 bg-purple-600 text-white">Send</button>
            </div>
        </form>
    </div>
</div>
