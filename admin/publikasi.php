...
$pageTitle = 'Lecturer Publications';
$db = new Database();
...
if (isset($_POST['multi_delete']) && !empty($_POST['ids'])) {
    ...
    $success = "Selected publications successfully deleted!";
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['multi_delete'])) {
    ...
    if (!$id_dosen || !$judul || !$tahun || !$jenis) {
        $error = 'Lecturer, title, year, and type must be filled in!';
    }
    ...
    if (!$error) {
        if ($id > 0) {
            $check = $db->query("SELECT id FROM publikasi WHERE id = $id LIMIT 1");
            if ($db->numRows($check) == 0) {
                $error = 'Publication data not found!';
            } else {
            ...
            if ($db->query($sql)) {
                $success = 'Publication successfully added!';
            } else {
...
if (!$dosenId) {
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        <h3 class="text-lg font-bold mb-4">Select Lecturers to Manage Publications</h3>
        <?php if ($dosens): ?>
            <ul class="space-y-2">
        ...
        <?php else: ?>
            <p>No lecturer data available. Please add first in the Organizational Structure.</p>
        <?php endif; ?>
    </div>
    <?php
...
<div class="bg-white rounded-xl shadow-md p-6 mb-6">
    <h3 class="text-lg font-bold mb-4">Publications: <?php echo htmlspecialchars($dosen['nama'] ?? ''); ?> (<?php echo htmlspecialchars($dosen['deskripsi'] ?? '-'); ?>)</h3>
    ...
    <form method="POST">
        ...
        <div class="flex justify-end">
            <?php if ($editData): ?>
                <a href="publikasi.php?dosen_id=<?= $dosenId ?>" class="px-4 py-2 bg-gray-300 rounded-lg mr-2">Cancel</a>
            <?php endif; ?>
            ...
            <table class="w-full">
                <thead class="bg-gray-100 text-left">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-sm font-semibold">Year</th>
                        <th class="px-6 py-3 text-sm font-semibold">Type</th>
                        <th class="px-6 py-3 text-sm font-semibold">Publisher</th>
                ...
                <tbody class="divide-y divide-gray-200">
                        ...
                        <?php foreach ($pubs as $row): ?>
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    <a href="?dosen_id=<?= $dosenId ?>&delete=<?= $row['id'] ?>" onclick="return confirm('Hapus publikasi ini?')" class="px-3 py-1 bg-red-500 text-white rounded-lg text-sm">Delete</a>
                                </td>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">Not publications available</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<div class="mt-4">
    <a href="struktur.php" class="px-4 py-2 bg-gray-300 rounded-lg">Back to Structure</a>
</div>
