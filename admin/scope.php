...
if (isset($_GET['delete'])) {
    ...
    if ($db->query("DELETE FROM scope WHERE id = $id")) {
        $success = "Scope successfully deleted!";
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        ...
        if ($db->query($sql)) {
            $success = $id > 0 ? "Scope successfully updated!" : "Scope successfully added!";
        } else {
...
<div class="bg-white rounded-xl shadow-md p-6 mb-8">
    ...
    <form method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div>
                <label class="font-semibold">Color (Hex)</label> <br>
        ...
        <div class="flex justify-end">
            <?php if ($editData): ?>
                <a href="scope.php" class="px-4 py-2 bg-gray-400 text-white rounded-lg">Cancel</a>
            <?php endif; ?>
    ...
    <div class="bg-white rounded-xl shadow-lg p-5 hover:shadow-xl transition overflow-hidden">
        ...
        <div class="flex space-x-2">
            ...
            <a href="?delete=<?= $row['id'] ?>"
               ...
               class="flex-1 px-3 py-2 bg-red-500 text-white rounded-lg text-center">Delete</a>
        </div>
    </div>
<?php endforeach; ?>
</div>
