...
$pageTitle = 'Vision & Mision of Laboratory';
$db = new Database();
    ...
    <form method="POST" action="">
        <div class="mb-6">
            ...
            <textarea name="visi" rows="4" required 
                      ...
                      placeholder="Enter the laboratory vision..."><?php echo htmlspecialchars($data['visi'] ?? ''); ?></textarea>
        </div>
        
        <div class="mb-6">
            <label class="block text-gray-700 font-semibold mb-2">
                <i class="fas fa-bullseye mr-2"></i>Mision
            </label>
            <textarea name="misi" rows="8" required 
                      class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-purple-500 focus:ring-2 focus:ring-purple-200 outline-none transition"
                      placeholder="Enter the laboratory mission (separate each point with an enter)..."><?php echo htmlspecialchars($data['misi'] ?? ''); ?></textarea>
            <p class="text-sm text-gray-600 mt-2">
                <i class="fas fa-info-circle mr-1"></i>Tip: Use numbered formats (1., 2., etc.) for each mission point.
            </p>
        </div>
